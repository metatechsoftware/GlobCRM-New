    ...
    3. **FeedPreviewWidgetComponent** (`widgets/feed-preview-widget/feed-preview-widget.component.ts`):
       ...
       - Template:
         ...
         - Compact feed item list (5 items):
           ...
           - Entity name: use `<app-preview-entity-link [entityType]="item.entityType" [entityId]="item.entityId" [entityName]="item.entityName">` shared component for entity name links (handles both normal click → preview sidebar AND Ctrl/Cmd+click → detail page navigation per locked decision). Import `PreviewEntityLinkComponent` in the component.
  ...
  <action>
    1. **NotificationDigestWidgetComponent** (`widgets/notification-digest-widget/notification-digest-widget.component.ts`):
       ...
       - Template:
         ...
         - Notification items with entityType/entityId: use `<app-preview-entity-link>` for any entity references in notification items (handles both normal click → preview sidebar AND Ctrl/Cmd+click → detail page navigation per locked decision)
    ...
    2. **RecentRecordsWidgetComponent** (`widgets/recent-records-widget/recent-records-widget.component.ts`):
       ...
       - Template:
         ...
         - List of 8 items:
           - Each: entity type icon (from EntityTypeRegistry — business for Company, people for Contact, handshake for Deal, etc.), entity name using `<app-preview-entity-link>` shared component (handles normal click → preview AND Ctrl/Cmd+click → detail page per locked decision), entity type label in muted text, relative timestamp
