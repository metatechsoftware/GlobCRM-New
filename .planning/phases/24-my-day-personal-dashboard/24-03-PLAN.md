...
@globcrm-web/src/app/shared/components/entity-preview/preview-entity-link.component.ts (entity click -> preview pattern; MUST enhance this component to support Ctrl/Cmd+click navigation to detail page)
</context>
...
<task type="auto">
  ...
  <action>
    0. **Enhance PreviewEntityLinkComponent** (`shared/components/entity-preview/preview-entity-link.component.ts`):
       - Add Ctrl/Cmd+click support per locked decision: "Entity name clicks inside widgets open preview sidebar, Ctrl/Cmd+click navigates to detail page"
       - Inject `Router` from `@angular/router`
       - Import `getEntityRoute` from `../../services/entity-type-registry`
       - Change template binding from `(click)="onClick()"` to `(click)="onClick($event)"`
       - Update `onClick(event: MouseEvent)` method:
         ```typescript
         ...
         ```
       - This enhancement is used by all My Day widgets AND existing feed entity links, so it's a shared improvement
    ...
    2. **TasksWidgetComponent** (`widgets/tasks-widget/tasks-widget.component.ts`):
       ...
       - Template:
         ...
         - Entity name link: Use `<app-preview-entity-link>` shared component for linked entity names. This handles both normal click (opens preview sidebar) AND Ctrl/Cmd+click (navigates to detail page). The component already exists at `shared/components/entity-preview/preview-entity-link.component.ts`. Pass `[entityType]="task.linkedEntityType"`, `[entityId]="task.linkedEntityId"`, `[entityName]="task.linkedEntityName"`. NOTE: If `PreviewEntityLinkComponent` does not yet support Ctrl/Cmd+click navigation, enhance its onClick method to check `$event.ctrlKey || $event.metaKey` and use Router.navigate to the entity detail page via `getEntityRoute()` (which it already imports indirectly via entity-type-registry). Update the template binding: `(click)="onClick($event)"` and the method signature to `onClick(event: MouseEvent)`. If ctrlKey/metaKey is true, call `router.navigate([getEntityRoute(entityType, entityId)])` instead of opening preview sidebar.
    ...
    4. **Update MyDayComponent** (replace placeholder from 24-01):
       ...
       - Component methods:
         ...
         - `onEntityClicked(event: { type: string; id: string })`: call `this.previewSidebarStore.open({ entityType: event.type, entityId: event.id })` — PreviewSidebarStore.open() takes a single PreviewEntry object `{ entityType, entityId, entityName? }`, NOT two positional arguments. NOTE: For entity name links inside widgets, prefer using the `<app-preview-entity-link>` shared component directly in widget templates (handles both normal click and Ctrl/Cmd+click). The entityClicked output is still needed for widgets that don't use the shared component (e.g., event cards).
