...
must_haves:
  truths:
    ...
    - "Note form pre-fills entityType, entityId, and entityName from queryParams when navigating from entity detail pages"
  artifacts:
    ...
    - path: "globcrm-web/src/app/features/notes/note-form/note-form.component.ts"
      provides: "Note create/edit form with rich text editor and queryParam pre-fill"
  ...
  key_links:
    ...
    - from: "note-form.component.ts"
      ...
      via: "Reads entityType, entityId, entityName queryParams in create mode"
      pattern: "queryParamMap.*entityType|queryParamMap.*entityId"
---
...
<task type="auto">
  ...
  <action>
    ...
    - Inject ActivatedRoute for both route params and query params
    ...
    - QueryParam pre-fill for create mode: In ngOnInit (or constructor), when in create mode (no :id route param):
      - Read entityType from queryParams: `this.route.snapshot.queryParamMap.get('entityType')`
      - Read entityId from queryParams: `this.route.snapshot.queryParamMap.get('entityId')`
      - Read entityName from queryParams: `this.route.snapshot.queryParamMap.get('entityName')`
      - If any of these are present, patchValue the FormGroup: `this.form.patchValue({ entityType, entityId, entityName })`
      - This enables Plan 07's "Add Note" button from entity detail pages (navigates to /notes/new?entityType=company&entityId={id}&entityName={name}) to pre-fill the entity fields
  ...
  <done>Notes list page shows paginated DynamicTable with saved views. Note form supports rich text editing with entity linking and pre-fills entityType/entityId/entityName from queryParams when navigating from entity detail pages. Note detail renders full HTML content. Routes configured for list, create, edit, detail.</done>
</task>
...
- NoteFormComponent reads entityType, entityId, entityName from queryParams and pre-fills form in create mode
...
<success_criteria>
Complete Notes frontend feature with list (DynamicTable), form (rich text via ngx-quill with entity queryParam pre-fill), detail page, signal store, and routes. Rich text editor available as shared component for reuse.
</success_criteria>
