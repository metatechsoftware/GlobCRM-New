...
must_haves:
  ...
  key_links:
    ...
    - from: "WorkflowsController"
      to: "IMemoryCache"
      via: "Direct cache key removal on CRUD mutations"
      pattern: "_cache\\.Remove.*workflow_active"
---
...
**IMPORTANT — Cache invalidation:** This plan runs in parallel with 19-02, so `WorkflowDomainEventHandler` does NOT exist yet. The controller MUST inject `IMemoryCache` directly (not `WorkflowDomainEventHandler`) and remove cache keys on every mutation. Use a private helper:
```csharp
...
```
Call this helper on Create, Update, Delete, status toggle, activate, and deactivate endpoints. The `WorkflowDomainEventHandler` (created by 19-02) will read these same cache keys, so the invalidation is compatible without a compile-time dependency.
...
3. `POST /api/workflows` — Create workflow
   ...
   - Call `InvalidateWorkflowCache(tenantId, entityType)` to clear IMemoryCache
...
6. Cache invalidation via IMemoryCache called on all workflow mutations (no compile-time dependency on WorkflowDomainEventHandler)
