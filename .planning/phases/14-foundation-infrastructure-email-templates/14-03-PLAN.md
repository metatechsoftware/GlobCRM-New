...
must_haves:
  truths:
    ...
    - "Merge fields render as entity-color-coded chip/badge pills inside the Unlayer editor (Contact=blue, Company=green, Deal=orange, Lead=purple) — visually distinct from regular text"
...
<task type="auto">
  ...
  <action>
    ...
    **Unlayer editor options (`editorOptions`):**
    - `mergeTags`: Build from store.mergeFields signal. **CRITICAL (locked decision):** Each entity group MUST include a `color` property so that merge fields render as entity-color-coded chip/badge pills inside the Unlayer editor — visually distinct from regular text. Unlayer natively supports the `color` property on merge tag groups. Structure:
      ```typescript
      {
        contact: {
          name: 'Contact',
          color: '#2196F3',  // Blue — matches merge field panel chip color
          mergeTags: {
        ...
        company: { name: 'Company', color: '#4CAF50', mergeTags: { ... } },  // Green
        deal: { name: 'Deal', color: '#FF9800', mergeTags: { ... } },        // Orange
        lead: { name: 'Lead', color: '#9C27B0', mergeTags: { ... } }         // Purple
      }
      ```
    - The `color` property on each group makes Unlayer render inserted merge tags as colored pill/badge chips (e.g., a blue "[Contact: First Name]" pill) that are visually distinct from regular text. This satisfies the locked user decision.
