...
<task type="auto">
  <name>Task 1: Extract remaining 54 feature template strings across 16 files</name>
  <files>
    globcrm-web/src/assets/i18n/activities/en.json
    globcrm-web/src/assets/i18n/activities/tr.json
    globcrm-web/src/assets/i18n/deals/en.json
  ...
  <action>
    The 54 remaining feature template strings are scattered across 16 files in 13 feature scopes. Most are aria-labels, back-button labels, and small strings missed during the initial extraction.
    ...
    **Workflows (10 strings, 3 files):**
    ...
    - Add to workflows/en.json: `builder.branchYes`, `builder.canvas.fitToView`, `builder.canvas.zoomIn`, `builder.canvas.zoomOut`, `builder.toolbar.entityTypes.activity`, etc.
    ...
    **Turkish translations for aria-labels / accessibility strings:**
    ...
    - Delete -> Sil
    - Download -> Indir
    - Fit to view -> Ekrana sigdir
    - Zoom in -> Yakinlastir
    - Zoom out -> Uzaklastir
    - Template preview -> Sablon onizleme
    - Swap primary record -> Birincil kaydi degistir
  </action>
  <verify>
    1. Run `cd globcrm-web && npx ng build --configuration development 2>&1 | tail -5` — must compile without errors
    2. Run `cd globcrm-web && npm run check:i18n` — key parity passes for all scopes
  </verify>
  <done>All 54 remaining feature template strings extracted to their respective scope JSON files. All 13 feature scope pairs (en/tr) maintain key parity. Angular build compiles successfully.</done>
</task>
...
<task type="auto">
  ...
  <action>
    **Step 1: Regenerate the i18n baseline.**
    ...
    This regenerates i18n-baseline.json with only the remaining hardcoded strings that still exist after plans 28-08, 28-09, and Task 1 of this plan.
...
6. Switching to Turkish renders all UI strings in Turkish across all feature areas
</verification>
