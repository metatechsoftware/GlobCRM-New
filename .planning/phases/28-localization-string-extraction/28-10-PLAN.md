...
<task type="auto">
  <name>Task 1a: Extract feature template strings for activities, deals, quotes, requests, contacts, companies, leads, products (34 strings, 10 files)</name>
  <files>
    globcrm-web/src/assets/i18n/activities/en.json
    globcrm-web/src/assets/i18n/activities/tr.json
    globcrm-web/src/assets/i18n/deals/en.json
  ...
  <action>
    Extract 34 hardcoded strings from 10 component files across 8 feature scopes (activities, deals, quotes, requests, contacts, companies, leads, products). These are primarily aria-labels, back-button labels, and small missed strings.
  ...
  <verify>
    ...
    2. Run `cd globcrm-web && npm run check:i18n` — key parity passes for activities, deals, quotes, requests, contacts, companies, leads, products scopes
  </verify>
  <done>All 34 hardcoded strings in activities, deals, quotes, requests, contacts, companies, leads, and products templates extracted to their respective scope JSON files. All 8 feature scope pairs (en/tr) maintain key parity. Angular build compiles successfully.</done>
</task>

<task type="auto">
  <name>Task 1b: Extract feature template strings for duplicates, auth, email-templates, sequences, workflows (20 strings, 6 files)</name>
  <files>
    globcrm-web/src/assets/i18n/duplicates/en.json
    globcrm-web/src/assets/i18n/duplicates/tr.json
    globcrm-web/src/assets/i18n/auth/en.json
  ...
  <action>
    Extract 20 hardcoded strings from 6 component files across 5 feature scopes (duplicates, auth, email-templates, sequences, workflows). These are the remaining scopes not covered in Task 1a.

    **Approach:** Same as Task 1a — for each file, read the baseline entries, check existing scope JSON, add missing keys, wire transloco pipe.
  ...
  <verify>
    1. Run `cd globcrm-web && npx ng build --configuration development 2>&1 | tail -5` — must compile without errors
    2. Run `cd globcrm-web && npm run check:i18n` — key parity passes for duplicates, auth, email-templates, sequences, workflows scopes
  </verify>
  <done>All 20 hardcoded strings in duplicates, auth, email-templates, sequences, and workflows templates extracted to their respective scope JSON files. All 5 feature scope pairs (en/tr) maintain key parity. Angular build compiles successfully.</done>
</task>
...
<task type="auto">
  ...
  <action>
    **Step 1: Regenerate the i18n baseline.**
    ...
    This regenerates i18n-baseline.json with only the remaining hardcoded strings that still exist after plans 28-08, 28-09, and Tasks 1a/1b of this plan.
