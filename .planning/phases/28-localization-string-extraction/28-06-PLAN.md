...
4. Also check the `features/notifications/` directory if it exists as a feature route (distinct from the shared notification-center component). If it has components, create a notifications scope or add to global keys as appropriate.
...
Run `cd globcrm-web && npx ng build --configuration development 2>&1 | head -50`. Verify scope wiring. Verify EN/TR key parity for each new scope.
  ...
  <done>Import and duplicates scopes created with complete EN/TR translations, routes wired, all templates using transloco pipe. Any notifications feature components also handled. Angular builds without errors.</done>
</task>
