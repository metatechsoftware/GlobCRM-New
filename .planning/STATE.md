...
- v1.3: 32 plans across 5 phases (Phase 27-31)
...
- 30-04: Column reorder uses separate CdkDropList (not inside CdkDropListGroup) to avoid CDK conflict between card and column drag systems
- 30-04: Float midpoint strategy for card sort order avoids needing to re-index all cards on each move
...
- 31-03: Merge tags hardcoded in component (not loaded from API) for immediate editor availability; API endpoint available for future dynamic loading
