...
<task type="auto">
  ...
  <action>
    ...
    3. **GET endpoint** `[HttpGet] GetMyDay()`:
       ...
       - **Pipeline query**: `Deals` where `OwnerId == userId`. Include Stage navigation property. Filter out terminal stages: `!d.Stage.IsWon && !d.Stage.IsLost`. IMPORTANT: Deal entity has NO Status property — won/lost is tracked via PipelineStage.IsWon and PipelineStage.IsLost boolean flags. Query: `_db.Deals.Include(d => d.Stage).Where(d => d.OwnerId == userId && !d.Stage.IsWon && !d.Stage.IsLost)`. GroupBy Stage. Select StageName, Color (from PipelineStage), DealCount, TotalValue (sum of Value). NOTE: GroupBy on navigation property may need .Select() first to materialize — fetch deals with stage info, then group in memory if EF Translation fails.
