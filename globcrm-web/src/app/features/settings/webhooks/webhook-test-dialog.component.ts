...
@Component({
  ...
  `,
  template: `
    <h2 mat-dialog-title>{{ 'settings.webhooks.testDialog.title' | transloco }}</h2>
    <mat-dialog-content>
      @if (loadingPreview()) {
        ...
        <div class="test-dialog__success">
          ...
          <p>{{ 'settings.webhooks.testDialog.sentSuccess' | transloco }}</p>
        </div>
      } @else if (previewPayload()) {
        <p style="font-size: 14px; color: var(--color-text-secondary)">
          {{ 'settings.webhooks.testDialog.previewDesc' | transloco }}
        </p>
    ...
    <mat-dialog-actions align="end">
      @if (sent()) {
        <button mat-flat-button color="primary" mat-dialog-close>{{ 'settings.webhooks.testDialog.close' | transloco }}</button>
      } @else {
        <button mat-button mat-dialog-close [disabled]="sending()">{{ 'settings.webhooks.testDialog.cancel' | transloco }}</button>
        @if (previewPayload() && !error()) {
          <button mat-flat-button color="primary"
            ...
            {{ 'settings.webhooks.testDialog.send' | transloco }}
          </button>
        }
      }
    </mat-dialog-actions>
